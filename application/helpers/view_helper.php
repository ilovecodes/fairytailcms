...
<?php
	...
	**/
	...
	function get_theme_name() {
		$theme = new Theme();
		return $theme -> get_selected_theme();
	...
	function load_navigation_variables() {
		...
		if(logged_in()) {
			$username = array('label' => username(), 'link' => $link_prefix.'welcome/viewdashboard');
	...
	function load_logo($theme_name = null) {
		$CI =& get_instance();
		$CI -> mysmarty -> assign('group', anchor(base_url(),'fairytail'));
		$CI -> mysmarty -> assign('tagline', "bla bla bla");
		if(!$theme_name) {
			$theme_name = get_theme_name();
		}
